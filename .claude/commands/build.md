...
2. Verify the packet's Figma URL is present and matches the user's URL when one is provided.
3. Do not attempt live Figma tool calls. Design data is delivered on disk before this repo is cloned, and visual comparison runs later in the platform-side audit.
...
4. **Phase 2** — Build all components. For EACH section:
   ...
   c. Use the pre-resolved screenshot reference in the hydration packet if present; if absent, rely on the platform-side post-build Gemini visual audit to catch visual divergence
